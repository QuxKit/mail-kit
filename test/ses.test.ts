...
describe('mail-kit/ses: sigv4', () => {
  ...
  it('sorts and encodes the query string and carries a session token', () => {
    const headers = signV4({
      ...
      service: 'service',
      region: 'us-east-1',
      credentials: { accessKeyId: 'AKIDEXAMPLE', secretAccessKey: 'wJalrXUtnFEMI/K7MDENG+bPxRfiCYEXAMPLEKEY', sessionToken: 'tok' },
      now: new Date('2015-08-30T12:36:00Z'),
    });
...
function fakeFetch(handler: (url: string, init: FetchInit) => { status: number; body?: unknown; headers?: Record<string, string> }) {
...
describe('mail-kit/ses: transport', () => {
  ...
  it('sends raw MIME through SendEmail with a per-tenant configuration set and tags', async () => {
    ...
    const t = sesTransport({ region: 'eu-west-1', credentials: creds, fetch: f.fetch, configurationSet: (e) => `tenant-${e.tenantId}` });
    ...
    assert.match(call.init.headers.authorization!, /^AWS4-HMAC-SHA256 Credential=AKIA\/\d{8}\/eu-west-1\/ses\/aws4_request/);
  ...
  it('maps errors: 429/5xx retryable, MessageRejected not', async () => {
    const throttled = sesTransport({ region: 'us-east-1', credentials: creds, fetch: fakeFetch(() => ({ status: 429, body: { message: 'slow down' }, headers: { 'x-amzn-errortype': 'TooManyRequestsException' } })).fetch });
    await assert.rejects(throttled.send(envelope), (e: unknown) => MailError.hasCode(e, 'transport') && e.failure.retryable === true && e.failure.status === 429);
    const rejected = sesTransport({ region: 'us-east-1', credentials: creds, fetch: fakeFetch(() => ({ status: 400, body: { message: 'Email address is not verified.' }, headers: { 'x-amzn-errortype': 'MessageRejected' } })).fetch });
    await assert.rejects(rejected.send(envelope), (e: unknown) => MailError.hasCode(e, 'transport') && e.failure.retryable === false && /MessageRejected/.test(e.failure.detail));
    const down = sesTransport({ region: 'us-east-1', credentials: creds, fetch: async () => { throw new Error('ECONNRESET'); } });
    await assert.rejects(down.send(envelope), (e: unknown) => MailError.hasCode(e, 'transport') && e.failure.retryable === true);
  });

  it('registers a domain: identity + mail-from, and returns the CNAME/MX/TXT checklist', async () => {
    const f = fakeFetch((url, init) => {
      if (init.method === 'POST' && url.endsWith('/v2/email/identities')) return { status: 200, body: { DkimAttributes: { Tokens: ['t1', 't2', 't3'] } } };
      if (init.method === 'PUT' && url.endsWith('/mail-from')) return { status: 200, body: {} };
      if (init.method === 'GET') return { status: 200, body: { VerifiedForSendingStatus: true, DkimAttributes: { Status: 'SUCCESS' }, MailFromAttributes: { MailFromDomainStatus: 'SUCCESS' } } };
      return { status: 404 };
    });
    ...
    assert.deepEqual(reg.records.map((r) => [r.type, r.name, r.value]), [
    ...
    assert.deepEqual(await t.checkDomain!('example.com', 'example.com'), { verified: true, detail: 'sending=true dkim=SUCCESS mailfrom=SUCCESS' });
  });

  it('re-reads an identity that already exists instead of failing', async () => {
    const f = fakeFetch((url, init) => {
      if (init.method === 'POST') return { status: 400, body: { message: 'already exists' }, headers: { 'x-amzn-errortype': 'AlreadyExistsException' } };
...
describe('mail-kit/ses: events', () => {
  ...
  it('parses bounce, complaint, delivery, delay, open and click, from the SNS envelope or the inner message', () => {
    const bounce = parseSesEvents({ Type: 'Notification', Message: JSON.stringify({ eventType: 'Bounce', mail, bounce: { bounceType: 'Permanent', bounceSubType: 'General', bouncedRecipients: [{ emailAddress: 'bob@example.org', diagnosticCode: 'smtp; 550 no such user' }], timestamp: '2026-08-16T12:00:05.000Z' } }) });
    ...
    const soft = parseSesEvents({ notificationType: 'Bounce', mail, bounce: { bounceType: 'Transient', bounceSubType: 'MailboxFull', bouncedRecipients: [{ emailAddress: 'bob@example.org' }] } });
    assert.equal(soft[0]!.bounce!.kind, 'soft');

    const complaint = parseSesEvents({ eventType: 'Complaint', mail, complaint: { complainedRecipients: [{ emailAddress: 'bob@example.org' }], complaintFeedbackType: 'abuse' } });
    assert.equal(complaint[0]!.type, 'complained');

    const delivered = parseSesEvents({ eventType: 'Delivery', mail, delivery: { recipients: ['bob@example.org'], timestamp: '2026-08-16T12:00:03.000Z' } });
    assert.equal(delivered[0]!.type, 'delivered');
    assert.equal(delivered[0]!.recipient, 'bob@example.org');

    const delayed = parseSesEvents({ eventType: 'DeliveryDelay', mail, deliveryDelay: { delayType: 'TransientCommunicationFailure', delayedRecipients: [{ emailAddress: 'bob@example.org' }] } });
    assert.equal(delayed[0]!.type, 'delayed');

    assert.equal(parseSesEvents({ eventType: 'Open', mail, open: { userAgent: 'UA' } })[0]!.userAgent, 'UA');
    assert.equal(parseSesEvents({ eventType: 'Click', mail, click: { link: 'https://x.example' } })[0]!.url, 'https://x.example');
  ...
  it('verifies an SNS signature and refuses a foreign certificate host', async () => {
    ...
    const canonical = ['Message', 'MessageId', 'Timestamp', 'TopicArn', 'Type'].map((f) => `${f}\n${(message as unknown as Record<string, string>)[f]}\n`).join('');
    ...
    await assert.rejects(verifySnsMessage({ ...message, Message: 'tampered' }, { fetch }), (e: unknown) => MailError.hasCode(e, 'signature_invalid'));
